...
//update user image
  ...
  if(isset($_POST['submitf'])) {
    $photof = new Media();
    $user_idf = (int)$_POST['user_id'];
    $photof->upload($_FILES['file_uploadf']);
    if($photof->process_userf($user_idf)){
      $session->msg('s','La foto fue subida al servidor.');
      redirect('edit_account.php');
      } else{
        $session->msg('d',join($photof->errors));
        redirect('edit_account.php');
      }
    }
?>
 ...
 //update user other info
  ...
  if(isset($_POST['updaten'])){
    $req_fields = array('namen' );
    validate_fields($req_fields);
    if(empty($errors)){
            $id = (int)$_SESSION['user_id'];
            $namen = remove_junk($db->escape($_POST['namen']));
            $sql = "UPDATE users SET nombre_almacen ='{$namen}'WHERE id='{$id}'";
            $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            $session->msg('s',"Cuenta actualizada. ");
            redirect('edit_account.php', false);
                ...
                <div class="col-md-6">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-md-3">
                                    <img class="img-circle img-size-2" src="uploads/users/<?php echo $user['fondo'];?>" alt="">
                                </div>
                                <div class="col-md-9">
                                    <form class="form" action="edit_account.php" method="POST" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <div class="col-md-4"></div>
                                            <input type="file" name="file_uploadf" multiple="multiple" class="btn btn-default btn-file" />
                                        </div>
                                        <div class="form-group">
                                            <input type="hidden" name="user_id" value="<?php echo $user['id'];?>">
                                            <button type="submit" name="submitf" class="btn btn-warning">Cambiar</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="panel panel-default">
                        <div class="panel-heading clearfix">
                            <span class="glyphicon glyphicon-edit"></span>
                            <span>Editar nombre del local</span>
                        </div>
                        <div class="panel-body">
                            <form method="post" action="edit_account.php?id=<?php echo (int)$user['id'];?>" class="clearfix">
                                <div class="form-group">
                                    <label for="name" class="control-label">Nombre del local</label>
                                    <input type="name" class="form-control" name="namen" value="<?php echo remove_junk(ucwords($user['nombre_almacen'])); ?>">
                                </div>
                                <div class="form-group clearfix">
                                    <button type="submit" name="updaten" class="btn btn-info">Actualizar</button>
                                </div>
                            </form>
                        </div>
                    </div>
